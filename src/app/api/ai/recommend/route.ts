...
export async function POST(req: NextRequest) {
  try {
    const { assetName, category, issueTitle, issueDescription, serviceHistory } =
      await req.json();

    const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
    
    if (!GEMINI_API_KEY) {
      // Dynamic fallback recommendation for hackathon presentation without API key
      const fallbackText = getFallbackRecommendation(category, issueTitle, issueDescription);
      return NextResponse.json({ success: true, recommendation: fallbackText, isMock: true }, { status: 200 });
    }
...
Keep response concise and actionable for a maintenance technician.`;
    ...
    try {
      ...
      return NextResponse.json({ success: true, recommendation: text }, { status: 200 });
    } catch (apiError) {
...
function getFallbackRecommendation(category: string, title: string, description: string): string {
  const cat = (category || "").toUpperCase();
  const t = (title || "").toLowerCase();
  const d = (description || "").toLowerCase();

  let rootCause = "Initial analysis indicates potential component wear or electrical signal discrepancy.";
  let steps = [
  ...
  let preventive = "Schedule quarterly diagnostic inspections and clean filters/coils to prevent recurrences.";
  let time = "1-2 Hours";
  let priority = "Medium";

  if (cat === "HVAC") {
    if (t.includes("leak") || d.includes("leak") || t.includes("water") || d.includes("water")) {
      rootCause = "Condensate drain line is likely clogged with algae or debris, causing water backup and overflow in the tray.";
      steps = [
      ...
      preventive = "Pour hot water with vinegar down the condensate line every 3 months and replace air filters monthly during peak seasons.";
      time = "45 - 60 Minutes";
      priority = "Medium";
    } else if (t.includes("cool") || d.includes("cool") || t.includes("heat") || d.includes("heat") || t.includes("blow") || d.includes("blow")) {
      rootCause = "Reduced thermal exchange, likely caused by dirty condenser coils, blocked ventilation vents, or low refrigerant level.";
      steps = [
      ...
      preventive = "Clean condenser coils annually, check electrical contacts, and ensure the outdoor unit has at least 2 feet of clear space around it.";
      time = "1.5 - 2 Hours";
      priority = "High";
    }
  } else if (cat === "ELECTRICAL") {
    priority = "High";
    if (t.includes("spark") || d.includes("spark") || t.includes("smoke") || d.includes("smoke") || t.includes("burn") || d.includes("burn")) {
      priority = "Critical";
      rootCause = "Arcing due to loose terminal connections, overloaded circuit, or severe short circuit in the outlet/switch.";
      steps = [
      ...
      preventive = "Conduct infrared thermography scans of electrical panels annually to detect hotspots and loose connections early.";
      time = "30 - 60 Minutes";
    } else {
      rootCause = "Intermittent power flow or tripped circuit breaker, possibly due to a ground fault or a faulty appliance load.";
      steps = [
      ...
      preventive = "Balance phase loads on the distribution panel and replace breakers showing signs of mechanical weakness.";
      time = "30 - 45 Minutes";
    }
  } else if (cat === "PLUMBING") {
    if (t.includes("clog") || d.includes("clog") || t.includes("drain") || d.includes("drain") || t.includes("block") || d.includes("block")) {
      rootCause = "Accumulation of organic waste, hair, grease, or foreign objects obstructing the drain trap or main pipe line.";
      steps = [
      ...
      preventive = "Install drain strainers to catch hair and debris, and never dump cooking grease or oil down any drain.";
      time = "30 - 60 Minutes";
      priority = "Medium";
    } else {
      rootCause = "Damaged pipe joint, worn washers, or high water pressure causing structural failure of fittings/seals.";
      steps = [
      ...
      preventive = "Monitor building water pressure (keep between 40-60 PSI) and inspect exposed pipes annually for corrosion.";
      time = "1 - 2 Hours";
      priority = "High";
    }
  } else if (cat === "IT EQUIPMENT" || cat === "IT") {
    rootCause = "Operating system deadlock, software configuration issue, or overheating due to accumulated dust.";
    steps = [
    ...
    preventive = "Implement automated software updates, ensure clean/cool server environments, and perform monthly backup tests.";
    time = "30 - 90 Minutes";
    priority = "Low";
  }
