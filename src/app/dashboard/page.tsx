...
import { QrCode, Ticket, CheckCircle, AlertTriangle, ChevronRight, Users, Calendar, Wrench } from "lucide-react";
...
interface StatsData {
  ...
  charts: {
    ...
    monthlyServiceRecords: Array<{ _id: { month: number; year: number }; count: number }>;
  };
}
...
export default function Dashboard() {
  ...
  if (!stats) return <div className="text-slate-400">Failed to render dashboard workspace.</div>;

  // Chart data formatting
  const priorityChartData = [
    { name: "Low", value: stats.charts.issuesByPriority.find(x => x._id === "low")?.count || 0, color: "#10b981" },
    { name: "Medium", value: stats.charts.issuesByPriority.find(x => x._id === "medium")?.count || 0, color: "#eab308" },
    { name: "High", value: stats.charts.issuesByPriority.find(x => x._id === "high")?.count || 0, color: "#f97316" },
    { name: "Critical", value: stats.charts.issuesByPriority.find(x => x._id === "critical")?.count || 0, color: "#ef4444" },
  ];

  const categoryChartData = stats.charts.assetsByCategory.map((cat, index) => {
    const colors = ["#22d3ee", "#a78bfa", "#3b82f6", "#10b981", "#fb923c", "#f472b6", "#e2e8f0"];
    return {
      name: cat._id || "Other",
      value: cat.count,
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "closed": return "bg-slate-700/30 text-slate-400";
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl relative overflow-hidden group hover:border-cyan-500/40 transition-all duration-300">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Asset Register</p>
              <h3 className="text-3xl font-extrabold text-white mt-2">{stats.assets.total}</h3>
            </div>
          ...
          <div className="mt-4 flex gap-3 text-xs text-slate-400 border-t border-slate-800/80 pt-4">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-400"></span> {stats.assets.operational} Good</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400"></span> {stats.assets.underMaintenance} Maint</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-400"></span> {stats.assets.faulty} Fault</span>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl relative overflow-hidden group hover:border-amber-500/40 transition-all duration-300">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-3xl font-extrabold text-white mt-2">{stats.issues.open}</h3>
            </div>
          ...
          <div className="mt-4 flex gap-3 text-xs text-slate-400 border-t border-slate-800/80 pt-4">
            <span className="text-red-400 font-bold">{stats.issues.critical} Critical tickets</span>
            <span className="text-emerald-400 font-bold">{stats.issues.resolved} Resolved total</span>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl relative overflow-hidden group hover:border-emerald-500/40 transition-all duration-300">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Service Logs</p>
              <h3 className="text-3xl font-extrabold text-white mt-2">{stats.serviceRecords}</h3>
            </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl relative overflow-hidden group hover:border-purple-500/40 transition-all duration-300">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-3xl font-extrabold text-white mt-2">{stats.technicians}</h3>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl lg:col-span-2">
          <h4 className="font-bold text-white mb-6">Incident Distribution by Priority</h4>
          <div className="h-80">
            {mounted ? (
              ...
              <div className="h-full flex items-center justify-center text-slate-600">Loading charts...</div>
            )}
          </div>
        </div>
              ...
              <>
                <div className="h-48 w-full relative">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                    <span className="text-2xl font-black text-white">{stats.assets.total}</span>
                    <span className="text-[10px] uppercase font-bold text-slate-500">Total Assets</span>
                  </div>
                </div>
                {/* Custom Legend */}
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mt-4 w-full text-xs text-slate-400">
                  {categoryChartData.slice(0, 6).map((cat, i) => (
                    <div key={i} className="flex items-center gap-2 truncate">
                      ...
                      <span className="truncate">{cat.name}: <strong className="text-white font-medium">{cat.value}</strong></span>
                    </div>
                  ))}
                </div>
              </>
            ) : (
              <div className="text-slate-500 text-sm">No assets registered yet. Add assets to see analysis.</div>
            )}
          </div>
        </div>
      </div>
                ...
                {stats.recentIssues.map((issue) => (
                  <tr key={issue._id} className="hover:bg-slate-800/20 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="font-semibold text-white">{issue.asset?.name || "Deleted Asset"}</div>
                      <span className="text-xs text-slate-500 font-mono">{issue.asset?.assetTag || "N/A"}</span>
                    </td>
                    <td className="px-6 py-4 font-medium text-slate-200">
                      {issue.title}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-bold border uppercase tracking-wider ${getPriorityBadge(issue.priority)}`}>
                        {issue.priority}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-0.5 rounded text-xs font-semibold uppercase ${getStatusBadge(issue.status)}`}>
